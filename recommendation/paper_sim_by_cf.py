...
def paperSim():
	connection = MySQLdb.connect (host = "127.0.0.1", user = "recop", passwd = "recocpgc", db = "recop")
	...
	for i, rels in simTable.items():
		...
		for j, weight in sorted(rels.items(), key=itemgetter(1), reverse=True):
			cursor.execute("replace into papersim_cf(src_id, dst_id, weight) values (%s,%s,%s);", (j, j, weight / math.sqrt(1 + ni[i] * ni[j])))
			k = k + 1
			if k > 20:
				break
